def run_interview():
    ...
    questions = [first_question] + random.sample(other_questions, 5)
    ...
    # FACE PROCESSING
        ...
        # QUESTION DISPLAY
        question_text = f"Q{current_question_index+1}: {questions[current_question_index]}"

        cv2.rectangle(frame, (20, 20), (frame.shape[1] - 20, 90), (0, 0, 0), -1)
        cv2.putText(frame,
                    question_text,
                    (30, 60),
                    cv2.FONT_HERSHEY_SIMPLEX,
        ...
        voice_wpm = voice_metrics["wpm"]
        voice_stability = voice_metrics["volume_stability"]
        voice_fillers = voice_metrics["filler_count"]

        cv2.rectangle(frame,
                      (10, frame.shape[0] - 100),
                      (360, frame.shape[0] - 10),
                      (0, 0, 0), -1)

        cv2.putText(frame, f"WPM: {voice_wpm:.0f}",
                    (20, frame.shape[0] - 70),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.6, (0, 255, 255), 2)

        cv2.putText(frame, f"Voice Stability: {voice_stability*100:.0f}%",
                    (20, frame.shape[0] - 45),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.6, (0, 255, 0), 2)

        cv2.putText(frame, f"Fillers: {voice_fillers}",
                    (20, frame.shape[0] - 20),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.6, (255, 200, 0), 2)
...
# SCORING SYSTEM
    ...
    # Normalize metrics
    engagement_score = float(avg_session_engagement) * 100
    wpm = float(voice_metrics["wpm"])
    fillers = int(voice_metrics["filler_count"])
    stability = float(voice_metrics.get("volume_stability", 0)) * 100

    # Speech score (ideal 120–160)
    if 120 <= wpm <= 160:
        speech_score = 100
    else:
        speech_score = max(0, 100 - abs(wpm - 140))

    # Filler penalty (each filler -5 points)
    filler_score = max(0, 100 - fillers * 5)
    ...
    # Confidence score (visual engagement)
    confidence_score = engagement_score

    # Final overall score
    overall_confidence = (
        0.6 * confidence_score +
        0.4 * communication_score   
    )
    # FINAL RESULTS
    return {
    "avg_engagement": float(avg_session_engagement) if frame_count > 0 else 0.0,
    "wpm": float(voice_metrics["wpm"]),
    "fillers": int(voice_metrics["filler_count"]),
